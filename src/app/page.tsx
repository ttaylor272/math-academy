    ...
    <main style={{ minHeight:'100vh', background:'#0f1117', display:'flex', alignItems:'center', justifyContent:'center', padding:'24px', fontFamily:'system-ui' }}>
      <div style={{ maxWidth:'520px', width:'100%' }}>
        <div style={{ textAlign:'center', marginBottom:'48px' }}>
          ...
          <div style={{ display:'inline-block', background:'linear-gradient(135deg,#6c63ff,#ff6b9d)', borderRadius:'20px', padding:'4px 16px', fontSize:'13px', fontWeight:700, color:'white', marginBottom:'8px' }}>
            ⚡ Advanced 6th Grade · 7th Grade Standards
          </div>
          <p style={{ color:'#8b93b8', fontSize:'15px', margin:0 }}>MCAP + MAP Prep for Tim & Jason</p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px', marginBottom:'24px' }}>
          {[{name:'Tim',path:'tim',emoji:'🧑‍🎓',color:'#6c63ff'},{name:'Jason',path:'jason',emoji:'👨‍🎓',color:'#ff6b9d'}].map(t => (
            <Link key={t.path} href={`/dashboard/${t.path}`} style={{ textDecoration:'none' }}>
              <div style={{ background:'#1a1d27', border:`2px solid #2e3350`, borderRadius:'20px', padding:'32px 24px', textAlign:'center', cursor:'pointer', transition:'border-color 0.2s' }}
                onMouseEnter={e=>(e.currentTarget as HTMLDivElement).style.borderColor=t.color}
